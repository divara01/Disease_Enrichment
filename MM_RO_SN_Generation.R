...
for (num_neighbors in 1:5){
  ...
  PAB_combined_Subnetworks <- Generate.MMandROCombined.SN(temp_df_B_SNs, temp_df_P_SNs, num_neighbors)
  
  lengths_PAB <- NULL
  for(i in 1:ncol(PAB_combined_Subnetworks)){
    lengths_PAB <- append(lengths_PAB, length(which(!is.na(PAB_combined_Subnetworks[,i]))))
  }
  delete <- max(lengths_PAB) + 1:nrow(PAB_combined_Subnetworks)
  PAB_combined_Subnetworks <- PAB_combined_Subnetworks[-delete, ]
  ...
  assign(paste("Situation1_B", neighbors_list[num_neighbors], "SNs", sep = "_"), temp_df_B_SNs)
  assign(paste("Situation1_A", neighbors_list[num_neighbors+5], "SNs", sep = "_"), temp_df_P_SNs)
  assign(paste("Situation1_PAB_combined_SN_d_", num_neighbors, sep = ""), PAB_combined_Subnetworks)
}
...
for (num_neighbors in 1:5){
  ...
  BAP_combined_Subnetworks <- Generate.MMandROCombined.SN(temp_df_B_SNs, temp_df_P_SNs, num_neighbors)
  
  lengths_BAP <- NULL
  for(i in 1:ncol(BAP_combined_Subnetworks)){
    lengths_BAP <- append(lengths_BAP, length(which(!is.na(BAP_combined_Subnetworks[,i]))))
  }
  delete <- max(lengths_BAP) + 1:nrow(BAP_combined_Subnetworks)
  BAP_combined_Subnetworks <- BAP_combined_Subnetworks[-delete, ]
  
  filename <- paste("Situation2_BAP_combined_SN_d_", num_neighbors, ".csv", sep = "")
  write.csv(BAP_combined_Subnetworks, file = filename)
  
  assign(paste("Situation2_B", neighbors_list[num_neighbors], "SNs", sep = "_"), temp_df_B_SNs)
  assign(paste("Situation2_A", neighbors_list[num_neighbors+5], "SNs", sep = "_"), temp_df_P_SNs)
  assign(paste("Situation2_BAP_combined_SN_d_", num_neighbors, sep = ""), BAP_combined_Subnetworks)
}

####Disease Enrichment on Combined Subnetworks####
filename = "DiseaseEnrichment_Situation1_PAB_combined_SN_d_5.csv"
subnetworks <- Situation1_PAB_combined_SN_d_5 #NEED TO CHANGE FILE DEGREE HERE
